...
class Map extends FlxSprite {
  ...
  public static inline function drawGridLines( state:PlayState, map:FlxSprite ):Void
  {
    ...
    for( x in 0...GRID_LINES_X ) {
      var x_line = FlxSpriteUtil.drawLine( map, ( x * GRID_SIZE ) + GRID_SIZE , TOP_BAR_BUFFER, ( x * GRID_SIZE ) + GRID_SIZE, Main.STAGE_HEIGHT, { color: FlxColor.RED, thickness: 0.5 } );
      x_line.immovable = true;
      state.add( x_line );
    }
    for( y in 0...GRID_LINES_Y ) {
      var y_line = FlxSpriteUtil.drawLine( map, 0, TOP_BAR_BUFFER + ( y * GRID_SIZE ) + GRID_SIZE , Main.STAGE_WIDTH, TOP_BAR_BUFFER + ( y * GRID_SIZE ) + GRID_SIZE , { color: FlxColor.RED, thickness: 0.5 } );
  ...
  public static inline function drawTopBar( state:PlayState, map:FlxSprite ):Void
  {
    ...
    topBar.makeGraphic(Main.STAGE_WIDTH, GRID_SIZE, FlxColor.GRAY);
    ...
    hero1Text.setFormat( AssetPaths.CHUNKY_FONT, 18, FlxColor.BLACK, FlxTextAlign.LEFT, FlxTextBorderStyle.SHADOW, FlxColor.GRAY, true);
    hero2Text.setFormat( AssetPaths.CHUNKY_FONT, 18, FlxColor.BLACK, FlxTextAlign.LEFT, FlxTextBorderStyle.SHADOW, FlxColor.GRAY, true);
    state.add( hero1Text );
    state.add( hero2Text );
  }
}
